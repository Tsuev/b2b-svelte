...
<tr>
    <td>{item.title}</td>
    <td>
        <input bind:value={item.quantity} on:change={() => changeSum(item.price, item.quantity)} type="number" class="form-control mb-2 mr-5" id="inlineFormInputName2">    
    </td>
    <td class="d-none d-md-table-cell">{formatter(item.price)} руб.</td>
    <td class="table-action">{formatter(item.price * item.quantity)}  руб.</td>
    <td>
